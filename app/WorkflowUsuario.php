...
class WorkflowUsuario extends Model
{
    ...
    */
    ...
    public static function verificar($path){
        ...
        $work = DB::table('workflow_usuarios as wu')
            //->join('users as us','us.id','=','wu.id_users')
            ->select('wu.id_workflow')
            ->where('wu.path','=',$path)
            ->first();
        $admin = DB::table('users as u')
        ->join('workflows as w','w.id_users','=','u.id')
        ->select('u.token')
        ->where('w.id','=',$work->id_workflow)
        ->first();
        ...
        if($admin) {
            ...
            //     $tokens = array_prepend($tokens, $admin[$i]->token);
            // }
            $tokens = array_prepend($tokens, $admin->token);
            $message = array("message" => "Se ah modificado el contenido de workflow:" . $path);
                WorkflowController::send_notification($tokens, $message);
        }
        $tokens=array();
        ...
        return json_encode(array("tokens"=>$admin,"shared"=>$shared));
    }
